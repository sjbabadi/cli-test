class View {
  constructor() {
    ...
    this.events = {
      ...
      'select list done': function(handler) {
        ...
        self.delegate('click', self.elements.done_lists_section, 'dl', (e) => handler(e, {mode: 'done', filter: true})); 
  ...
  delegate(event, parent, childSelector, handler) {
    parent.addEventListener(event, (e) => {
      for(let tgt = e.target; tgt && tgt !== parent; tgt = tgt.parentNode) {
        if(tgt.matches(childSelector)) {
          handler(e);
        }
      }
    });
  ...
  renderCurrentList(context) {
    ...
    document.body.insertAdjacentHTML('afterbegin', this.templates['main_template'](context));
  ...
  _populateForm(data) {
    ...
    $form.find('select[name="month"]').val(data.month);
    // $form.find('select[name="year"]').val(data.year).change();
    // $form.find('textarea[name="description"]').val(data.description).change();
  }

  renderModal() {
    this.elements.modal_layer.style.display = 'block';
    this.elements.formDiv.style.display = 'block';
  }

  hideModal() {
    this.elements.modal_layer.style.display = 'none';
    this.elements.formDiv.style.display = 'none'; 
  }
